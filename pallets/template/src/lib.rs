...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		Winners { winners: BoundedVec<T::AccountId, ConstU32<4>>},
	...
	pub(crate) type Winners<T: Config> =
		StorageValue<_, BoundedVec<T::AccountId, ConstU32<4>>>;
			...
			Lottery::<T>::mutate(|mut lottery| -> Weight {
				if let Some(config) = &mut lottery {
					...
					if payout_block <= n {
						...
						let mut winners: BoundedVec<T::AccountId, ConstU32<4>> = Default::default(); 
						let winners_may_exist = Self::choose_accounts(4);
						if let Some(winners_may_exist) = winners_may_exist{
							winners_may_exist.iter()
								.map(|winner| winner.clone().unwrap_or(lottery_account.clone()))	
								.for_each(|winner| {
									winners.try_push(winner);
								});
						}  

						Winners::<T>::put(winners.clone());
						
						Self::deposit_event(Event::<T>::Winners {winners});
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn buy_ticket(
			...
			let _ = Self::do_buy_ticket(&caller);
			Ok(())
		}
...
impl<T: Config> Pallet<T> {
	...
	fn do_buy_ticket(caller: &T::AccountId) -> DispatchResult {
		...
		Self::deposit_event(Event::<T>::TicketBought { who: caller.clone() });
	...
	fn choose_accounts(num_accounts: u32) -> Option<Vec<Option<T::AccountId>>> {
		match Self::choose_tickets(TicketsCount::<T>::get(), num_accounts) {
			None => None,
			Some(tickets) => {
				Some(
					tickets.iter().map(|ticket| Tickets::<T>::get(ticket)).collect()
				)
			}
		}
	}
	...
	fn choose_tickets(total: u32, num_tickets: u32) -> Option<Vec<u32>> {
		if total == 0 {
			return None
		}

		let mut tickets: Vec<u32> = Default::default(); 
		...
		for ticket in 0..num_tickets {
			...
			for i in 0..T::MaxGenerateRandom::get() {
				if random_number < u32::MAX - u32::MAX % total && !tickets.contains(&(random_number % total)) {
					break
				}
			...
			tickets.push(random_number % total);
		}
		Some(tickets)
	}
